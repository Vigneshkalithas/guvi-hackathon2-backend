...
app.post("/adminlogin", async (req, res) => {
  try {
    ...
    if (adminCheck) {
      ...
      if (match) {
        const token = jwt.sign({ _id: adminCheck._id }, SECRET, {
          expiresIn: "5m",
        });
        res.status(200).json({
...
app.put("/movies/:id", async (req, res) => {
  try {
    const connection = await MongoClient.connect(URL);
    const db = connection.db("hackathon");
    await db.collection("moviedatas").updateOne(
